...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ error: "A post id is required." }, { status: 422 });
  }
  ...
  try {
    const result = await callAgent<Record<string, unknown>>("/discussion-summary", {
      body: { post_id: parsed.data.post_id, requester_id: viewer.userId },
    });
    ...
    return NextResponse.json({ error: "The summary is unavailable." }, { status: 502 });
  }
}
